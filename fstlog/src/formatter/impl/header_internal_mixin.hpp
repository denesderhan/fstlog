...
namespace fstlog {
    //usable only with the internal decoder (endianness, alignment issues)
    template<typename L>
    class header_internal_mixin : public L {
    public:
        header_internal_mixin() noexcept = default;

        header_internal_mixin(const header_internal_mixin&) = delete;
        header_internal_mixin& operator=(const header_internal_mixin&) = delete;
        header_internal_mixin(header_internal_mixin&&) = delete;
        header_internal_mixin& operator=(header_internal_mixin&&) = delete;
